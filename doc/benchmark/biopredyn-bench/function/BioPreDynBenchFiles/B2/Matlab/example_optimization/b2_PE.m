% b2_PE               Perform an example of parameter estimation of the  
%                     b2 model, starting from the nominal parameters
%
...
% Load the file b2_bounds.mat, containing the following vectors:
% - Lower bounds for the parameters: lb
% - Upper bounds for the parameters: ub
% - Initial guess (nominal values, to be optimized): x0 
load b2_bounds

% Eval fobj at x0
disp('-- Evaluating objective function at initial guess x0')
[fobj_x0,constr_x0,residuals_x0] = b2_obj(x0); 
disp(fobj_x0)
...
% [x,fval] = fmincon(@b2_obj,x0,A,b,Aeq,beq,lb,ub); 
